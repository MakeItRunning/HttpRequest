...
public class ActivationCodeManageActivity extends BaseActivity implements OnClickListener{
	...
	private RelativeLayout agents_new_actcode,agents_actcode,agents_actcode1,apply_actcode;
...
//		phone = ((AppContext)getApplication()).getMobile();
	...
	private void initview() {
		...
		apply_actcode = (RelativeLayout) findViewById(R.id.apply_actcode);
		apply_actcode.setOnClickListener(this);
...
//			startActivityForResult(it1, REQUEST_CONTACT);
		...
		case R.id.lin_actcode:
			...
			startActivity(it);
			break;
		case R.id.apply_actcode:
			if (!no_generation.getText().equals("0")) {
				warnDialog = new OneButtonDialogWarn(ActivationCodeManageActivity.this,
						R.style.CustomDialog, "提示",
						"已有激活码", "确定",
						new OnMyDialogClickListener() {
				...
				Actcodemanage1  atask = new Actcodemanage1();
				atask.execute(HttpUrls.APPLYCODE + "", phone,agtid);
			}
			break;
			
		case R.id.agents_actcode1:
			Intent it1 = new Intent(ActivationCodeManageActivity.this,AgentListActivity1.class);
			it1.putExtra("num", setnum);
			it1.putExtra("tag", "0");
			startActivity(it1);
			break;
...
//					has_been_act.setText(result.get("ALLOTNUM").toString());
					...
					has_been_act.setText(Integer.parseInt(a)+Integer.parseInt(b)+"");
...
//							result.get(Entity.RSPMSG).toString());
	...
	class Actcodemanage1 extends
		...
		protected HashMap<String, Object> doInBackground(String... params) {
			String[] values = { params[0], params[1], params[2] };
			return NetCommunicate
					.getAgentMidatc(HttpUrls.APPLYCODE, values);
		}
		...
		protected void onPostExecute(HashMap<String, Object> result) {
			...
			if (result != null) {
				if (Entity.STATE_OK
					...
					String a = result.get("ACTIVENUM").toString();
					String b = result.get("ALLOTNUM").toString();
					has_been_act.setText(Integer.parseInt(a)
							+ Integer.parseInt(b) + "");
