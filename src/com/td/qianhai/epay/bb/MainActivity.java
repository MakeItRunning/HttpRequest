...
import com.td.qianhai.epay.bb.broadcast.DownloadService;
import com.td.qianhai.epay.bb.interfaces.DownloadListener;
import com.td.qianhai.epay.bb.mail.utils.MyCacheUtil;
import com.td.qianhai.epay.bb.push.Utils;
import com.td.qianhai.epay.bb.thread.VersionUpdateVerifyThread;
...
import com.td.qianhai.epay.bb.views.dialog.DownloadDialog;
import com.td.qianhai.epay.bb.views.dialog.DownloanDialog;
import com.td.qianhai.epay.bb.views.dialog.OneButtonImgDialog;
import com.td.qianhai.epay.bb.views.dialog.TwoButtonDialog;
import com.td.qianhai.epay.bb.views.dialog.interfaces.OnIsDownloadClickListener;
...
//                Utils.getMetaValue(MainActivity.this, "api_key"));
		...
		editor.putString("OEMID", "BbbaoOEM");
		editor.commit();
        
        AnalyticsConfig.setChannel("bbbao");
        settitle();
//        PushManager.startWork(getApplicationContext(),
//                PushConstants.LOGIN_TYPE_API_KEY,
//                Utils.getMetaValue(MainActivity.this, "api_key"));
...
//							if(share.getBoolean("isfirst",false)){
								Intent intent = new Intent(MainActivity.this,
										FmMainActivity.class);//UnlockLoginActivity.class
...
//							if(share.getBoolean("isfirst",false)){
								Intent intent = new Intent(MainActivity.this,
										FmMainActivity.class);//UserActivity.class
								startActivity(intent);
//							}else{
//								Intent intent = new Intent(MainActivity.this,
//										WelcomeActivity.class);
		...
		if (result != null) {
			if (Entity.STATE_OK.equals(result.get(Entity.RSPCOD))) {
				...
				int versionCode = Integer.parseInt(result.get("VERCODE")
						.toString());
				String apknote = "";
				if(result.get("APKNOTE")!=null){
					apknote = result.get("APKNOTE").toString();
				}
				if (getVersion() < versionCode) {
					if (android.os.Environment.MEDIA_MOUNTED
							.equals(android.os.Environment
						...
						if (isForce.equals("0")) {
							// 非强制更新
							flag = 0;
							showUpdateNoticeDialog(apknote);
							return;
...
//							startDowApk();
				...
				if(result.get(Entity.RSPCOD)!=null&&result.get(Entity.RSPCOD).equals("000002")){
					...
					if(flag == 1){
						...
						Intent it  =  new Intent(MainActivity.this,FmMainActivity.class);//UserActivity.class
						startActivity(it);
					}
...
//			startActivity(it);
	 ...
	 * 显示非强制更新的dialog
	 */
	private void showUpdateNoticeDialog(String a) {
		downloadDialog = new TwoButtonDialog(this, R.style.CustomDialog,
				"软件版本更新(请在wifi环境中进行)",a, "确认更新", "下次再说",
							...
							if(!StringUtil.isEmpty(share.getString("usermobile",""))){
								if(share.getBoolean("isfirst",false)){
									...
									startActivity(intent);	
										...
										Intent intent = new Intent(MainActivity.this,
												FmMainActivity.class );//WelcomeActivity.class
										intent.putExtra("type", "1");
										startActivity(intent);
