...
//	etLoginPasswdAgain
	/** 协议查看、验证码获取、提交注册 */
	private Button  btnGetVerifCode, btnSubmit;
	/** 同意协议 */
...
//		scrollView = (ScrollView) findViewById(R.id.btn_register_scrollview);
		...
		btnQueryAgreement.setText(Html.fromHtml("<u>用户协议</u>"));
		btnQueryAgreement.setOnClickListener(this);
		btnGetVerifCode = (Button) findViewById(R.id.btn_register_get_verif_code);
...
////
		if (actcode == null || actcode.equals("")) {
			ToastCustom.showMessage(this, "请输推荐人号码", Toast.LENGTH_SHORT);
			return;
		}
...
//		}
		...
		task.execute(mobile, loginPasswd, "", mermod, verifCode, trmtyp,HttpUrls.APPNUM,actcode,"","","");
	}
		...
		protected void onPostExecute(CityEntity result) {
			...
			if (result != null) {
				...
				if (Entity.STATE_OK.equals(result.getRspcod())) {
					...
					// ProvinceDialog dialog = new ProvinceDialog(3,
					// RegisterActivity.this, result.list,
					// new OnProvinceDialogListener() {
					//
					// @Override
