package com.td.qianhai.epay.bb.advertising;
...
//			if(mHeight>=310){
//				s=1.08;
//			}else if(mHeight>=290&&mHeight<310){
//				s=1.02;
//			}else if(mHeight<=290&&mHeight>250){
//				s= 1;
//			}else if(mHeight<250){
//				
//				s=1.1 ;
//			}
			...
			if (mCurrentImage >= 0 && mCurrentImage < mCount) {
				...
				for (int i = 0; i < items.length; i++) {
					...
					childView.layout(left + IMAGE_PADDING, -2,left
						+ mImageWidth - IMAGE_PADDING, mHeight);
					childView.initImageViewBitmap();
					LinearLayout.LayoutParams LP_FW = new LinearLayout.LayoutParams(  
			                LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.MATCH_PARENT);
					LP_FW.gravity = Gravity.TOP;
					
					childView.setLayoutParams(LP_FW);
//					
	...
	Runnable runnable = new Runnable() {
		...
		public void run() {
			...
			handlers.postDelayed(this, times);// 20是延时时长
		}
...
//				
				...
				handlers.postDelayed(runnable, times);
				if (mVelocityTracker != null) {
