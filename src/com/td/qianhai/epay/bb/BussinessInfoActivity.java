...
public class BussinessInfoActivity extends BaseActivity implements OnClickListener{
	...
	private LinearLayout lin_adress,lin_feerate,lin_region_agt,level;
...
//			String getpcard = card.substring(card.length()-4);
		...
		if(map.get("ISAREAAGENT")!=null){
			
			String isareaagent = map.get("ISAREAAGENT").toString();
		if(isareaagent.equals("1")){
			...
			tvCustonId.setText("大区代理");
		}else if(isareaagent.equals("5")){
			tvCustonId.setText("全国代理");
			}else{
		...
		if(map.get("FREEZETYPE")!=null){
			
			String free= map.get("FREEZETYPE").toString();
			if(free.equals("1")){
				tvCustonId.setText("区级代理");
			}else if(free.equals("2")){
				tvCustonId.setText("市级代理");
			}else if(free.equals("3")){
				tvCustonId.setText("省级代理");
			}else if(free.equals("4")){
				tvCustonId.setText("大区代理");
			}else if(free.equals("5")){
				tvCustonId.setText("全国代理");
				}else{
...
//					tv_region_agt.setText(map.get("AGTPROVNAM").toString()+toString()+map.get("AGTCITYNAM").toString());
//				}else{
//					lin_region_agt.setVisibility(View.GONE);
//				}
...
//				tvCustonId.setText("分销商");
		...
		if(map.get("AUTHENTICATIONNUM")!=null){
			tvStatus.setText("您是第"+map.get("AUTHENTICATIONNUM").toString()+"位实名认证的用户");
		}else{
		...
		if(map.get("CITYNAM")!=null&&map.get("PROVNAM")!=null&&map.get("AREANAM")!=null){
			...
			tv_bank_name_address.setText("暂无");
