...
public class RequestCardInfoChangeActivity extends BaseActivity {
	...
	/** 返回、title中间内容 */
	private TextView bt_Back, tv_title;
...
//		custId = ((AppContext) this.getApplication()).getCustId();
		psamId = ((AppContext) RequestCardInfoChangeActivity.this
				.getApplication()).getPsamId();
				...
				if (DateUtil.isFastDoubleClick()) {
					...
					if (tvBankName.getText().toString()==null||tvBankName.getText().toString().equals("")||tvBankName.getText().toString().equals("未知")) {
						ToastCustom.showMessage(
								getApplicationContext(), "请先填写正确的银行卡号",
								Toast.LENGTH_SHORT);
						return;
					}
					
					if(tvBankName.getText().length()>=0){
						ToastCustom.showMessage(
								getApplicationContext(), "获取银行名称",
								Toast.LENGTH_SHORT);
						return;
					}
		...
		protected void onPostExecute(HashMap<String, Object> result) {
			...
			if (result != null) {
				...
				if (result.get(Entity.RSPCOD).equals(Entity.STATE_OK)) {
					ToastCustom.showMessage(getApplicationContext(),"申请已提交,请上传资料等待后台审核",
					Toast.LENGTH_SHORT);
					...
					ToastCustom.showMessage(getApplicationContext(),result.get(Entity.RSPMSG).toString(),
							Toast.LENGTH_SHORT);
				}
			...
			if (result != null) {
				if (Entity.STATE_OK.equals(result.getRspcod())) {
					...
					if (bankname == null || bankname == "") {
						tvBankName
						.setText("未知");
						ToastCustom.showMessage(getApplicationContext(), "银行卡信息获取失败 ，请检卡号查输入是否正确", Toast.LENGTH_SHORT);
...
//						uBankCardNo.setText(result.get("ACTNO").toString());
					}
